if(location.href.includes("edit")) {
    let selectVariants = $('.select-variants').last();
    let getLastVariantId = selectVariants.attr('id').split('-');
    var currentIndex = parseInt(getLastVariantId[getLastVariantId.length - 1]);
} else  var currentIndex = 0;
var indexs = [];

$(document).ready(function () {
    if(currentIndex < 1) addVariantTemplate();
    else {
    ...
    $("#variant-sections").append(`<div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select id="select2-option-${currentIndex}" data-index="${currentIndex}" name="product_variant[${currentIndex}][option]" class="form-control select-variants custom-select select2 select2-option">
                                            <option value="1">
                                                Color
                                            </option>
                                            <option value="2">
